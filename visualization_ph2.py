...
class visualization():
    ...
    def bloat_eqns(self, shape, clearance):
        ...
        for i in range(len(shape)-1): #Find the equation of all the lines of rect1 obstacle      
            ...
            # print("For points ","(",rect1_n[i], rect1_n[i+1], ")","Bloated obstacle eqn: ", new_op[0],"x"," +", new_op[1],"y"," +",new_op[2]) 
            shape_b_eq.append(new_op) #Bloated rect obstacle eqn
    ...
    def map_half_plane(self, clearance): 
        #Normal Obstacle Space: (points taken in clock-wise order)
        rect1_n = [(150,200), (150,75), (165,75), (165,200), (150,200)]
        rect2_n = [(250,125), (250,0), (265,0), (265,125), (250,125)]
        cir_n = [(400,110), 50]
        cir_b_n = cir_n.copy()
        cir_b_n[1] += clearance
        wall_n = [(0, 0), (600,0), (600,200), (0,200), (0,0)] #Boundary of the map     
        
        self.image = np.zeros((600, 200, 3), np.uint8) #create a new image with a black background
        
        self.rect1_n_eq = self.normal_eqns(rect1_n)
        self.rect1_b_eq = self.bloat_eqns(rect1_n, clearance)

        self.rect2_n_eq = self.normal_eqns(rect2_n)
        self.rect2_b_eq = self.bloat_eqns(rect2_n, clearance)

        self.wall_n_eq = self.normal_eqns(wall_n)
        self.wall_b_eq = self.bloat_eqns(wall_n, -clearance)
        ...
        for i in range(0, 600): #Loop through all the nodes (pixels) in the map
            for j in range(0, 200): 
                ...
                elif (self.check_poly(cir_b_n, i, j) == True): #Check if the node is inside the bloated rect 1 obstacle space
                    if (self.check_poly(cir_n, i, j) == True): #Check if the node is inside the normal rect 1 obstacle space
                        self.image[i][j] = [255, 255, 255]
                    else:
                        self.image[i][j] = [255, 191, 0]
...
def main():
    ...
    image = map.map_half_plane(10) #Create the map
